...
def populate():
    try:
        ...
        for i in range(0, total_rows, batch_size):
            ...
            args_str = ','.join(cur.mogrify("(%s,%s,%s,%s)", x).decode('utf-8') for x in rows)
            cur.execute("INSERT INTO orders (user_id, status, content, created_at) VALUES " + args_str)
            conn.commit()
